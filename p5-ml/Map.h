...
private:
  ...
  class PairComp {
    private:
      Key_compare compKey; // instance of Key_compare

    public:
      bool operator() (Pair_type a, Pair_type b){
        return compKey(a.first, b.first); // keycompare
      }
  };
...
public:
  ...
  Value_type& operator[](const Key_type& k){
    if(mapTree.find({ k, Value_type() }) != mapTree.end()){
      return mapTree.find({ k, Value_type() })->second;
    }
    else{
      return mapTree.insert({ k, Value_type() })->second;
    }
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    if(mapTree.find(val) != mapTree.end()){
      return std::make_pair(mapTree.find(val), false);
    }
    else{
      return std::make_pair(mapTree.insert(val), true);
    }
  }
